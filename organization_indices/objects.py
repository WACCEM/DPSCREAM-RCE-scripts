...
# class make_objects :
#     def __init__(self, image):
#         """Process each 2D slice in the 3D image array, accumulating results."""
#         if image.ndim != 3:
#             raise ValueError("Expected a 3D array with shape (time, X, Y)")

#         self.labeled = []
#         self.regions = []
#         self.polynoms = []
#         self.area_skm = 0
#         self.area_spg = 0.5 # adjust to match expected polygon shape nuance
#         self.areas = []
#         self.centroids = []
#         self.diameters = []
#         self.perimeter = []
...
#             self.process_slice(image[t])
        
        
#     def process_slice(self, image_2d):
#         "Create the regions and polynoms, they have the same order"

#         image_2d    = ndimage.binary_fill_holes(image_2d).astype(int)  # fill holes
#         labeled_2d  = skm.label(image_2d, background=0 , connectivity=2) #connect also in diagonal (default is 1)
#         regions_2d  = skm.regionprops(labeled_2d)
...
#         self.labeled.append(labeled_2d)
#         self.regions.extend(regions_2d)
#         self.polynoms.extend(polynoms)

#         self.area_skm += np.sum([r.area for r in regions_2d])
#         self.area_spg += np.sum([p.area+0.5 for p in polynoms])
#         self.areas.extend([r.area for r in regions_2d])
#         self.centroids.extend([r.centroid for r in regions_2d])
#         self.diameters.extend([r.equivalent_diameter for r in regions_2d])
#         self.perimeter.extend(perimeter_)
#         self.number_of_objects += len(polynoms)
...
#     def compute_distance_centroids (self) :
#         for time_slices in range(len(self.objects.regions)):
#             # centroids = self.objects.centroids
#             centroids = self.objects.centroids[time_slices]
#             xs  = np.array([c[1] for c in centroids])
#             ys  = np.array([c[0] for c in centroids])
...
# #             # compute the nearest neighbour distance
# #             self.dist_min = np.nanmin(distances, axis=0) if self.number_of_objects > 1 else np.nan
#             self.centroids_x.append(xs)
#             self.centroids_y.append(ys)
#             self.distance_centroids_x.append(dist_x)
#             self.distance_centroids_y.append(dist_y)
#             self.distance_centroids.append(distances)

#             # compute the nearest neighbour distance
#             dist_min = np.nanmin(distances, axis=0) if self.objects.number_of_objects > 1 else np.nan
#             self.dist_min.append(dist_min)


#     def compute_distance_edges (self) :
#         for time_slices in range(len(self.objects.polynoms)):
#             distance_edges = np.empty((self.number_of_objects, self.number_of_objects))
#             for n in range(self.number_of_objects) :
#                 for m in range(n) :
#                     distance_edges[m, n] = self.objects.polynoms[time_slices][n].distance(self.objects.polynoms[time_slices][m])
#                     # distance_edges[m,n] = self.objects.polynoms[n].distance(self.objects.polynoms[m])
#                     distance_edges[n,m] = distance_edges[m,n]
#                     #if distance_edges[n,m]==0 : print('WARNING: distance between edges is zero')
#             np.fill_diagonal(distance_edges, np.nan)
#             # self.distance_edges =  distance_edges
#             self.distance_edges.append(distance_edges)
