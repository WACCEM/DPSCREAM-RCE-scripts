...
# --jobs 28: memory-limited by calc_cp peak RSS (~17 GB/job) on a 512 GB --exclusive node.
# floor(512 GB / 17 GB) = 30; use 28 to leave headroom for OS and NumPy buffers.
# Do NOT raise above 28 on 512 GB nodes or above 4 for interactive login-node testing.
#for input files with 24 time samples (e.g. 1hr frequency), calc_cp RSS is higher and can run only with 
# 8 jobs or smaller.
parallel --jobs 8 --colsep ' ' -a ${inputfile} ${pydir}/${runscript} {1} {2} {3} > ${logfile} 2>&1
