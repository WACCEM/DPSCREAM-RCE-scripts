...
#idct = 0 #for debugging
    ...
    if(idct == 0):
        _coord_vars = [v for v in ['lat', 'lon', 'lev'] if v in xr.open_dataset(day_files[0])] 

    ds_day = xr.open_mfdataset(day_files, combine='by_coords', parallel=False)[[varname] + _coord_vars]
    ...
    if(idct == 0):
        print("reading lat, lon, lev and checking initial time from dataset attributes...")
        #the original units "rad" is wrong
        lat = ds_day['lat'].isel(time=0, drop=True).assign_attrs({'units': 'm'}) if 'lat' in ds_day else None
        lon = ds_day['lon'].isel(time=0, drop=True).assign_attrs({'units': 'm'}) if 'lon' in ds_day else None
        lev = ds_day['lev'] if 'lev' in ds_day else None
    ...
    if(styear == init_year and stmonth == init_month and stday == init_day and iday == init_day):
