...
hist_file = "dx1km_L600km_RCE03_gpu.hist.AVERAGE.nhours_x1.2000-01-01-00000.nc"
scrip_file  = "scrip_DPSCREAM_RCE_dx1km_600x600km_plus.nc"
...
# Domain parameters will be dynamically inferred from the dataset
...
# Infer grid spacings and domain size from coordinates
dy = float(np.unique(np.diff(np.sort(np.unique(lat_m_in))))[0])
dx = float(np.unique(np.diff(np.sort(np.unique(lon_m_in))))[0])

Ly = float(lat_m_in.max() - lat_m_in.min() + dy)
Lx = float(lon_m_in.max() - lon_m_in.min() + dx)

# Infer dynamics grid spacing assuming pg2 (2 physics columns per element, 3 dynamics columns)
dx_dyn = dx * 2.0 / 3.0

print(f"Inferred from data: Lx={Lx}, Ly={Ly}, dx={dx}, dy={dy}, dx_dyn={dx_dyn:.1f}")
